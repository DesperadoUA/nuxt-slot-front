    ...
    <div class="container">
        <div class="detail-row">
            ...
            <div class="detail-row__content">
                <ul class="detail-list">
                    ...
                    <li class="detail-list__item">
                        <span class="detail-list__ttl">{{translates.FREE_SPINS[config.LANG]}}</span>
                        <span class="detail-list__value">{{ detailsFilters(value.free_spins) }}</span>
                    </li>
                    <li class="detail-list__item">
                        <span class="detail-list__ttl">{{translates.PAY_LINES[config.LANG]}}</span>
                        <span class="detail-list__value">{{ value.number_rows }}</span>
                    </li>
                    <li class="detail-list__item">
                        <span class="detail-list__ttl">Scatters</span>
                        <span class="detail-list__value">{{ detailsFilters(value.scatters) }}</span>
                    </li>
                    <li class="detail-list__item">
                        <span class="detail-list__ttl">{{translates.BONUS_ROUND[config.LANG]}}</span>
                        <span class="detail-list__value">{{ detailsFilters(value.bonus_rounds) }}</span>
                    </li>
                    <li class="detail-list__item">
                        <span class="detail-list__ttl">{{translates.WILD_SYMBOL[config.LANG]}}</span>
                        <span class="detail-list__value">{{ detailsFilters(value.wild_symbol) }}</span>
                    </li>
                </ul>
            </div>
        </div>
        ...
        <div class="detail-row">
            <div class="detail-row__heading" v-if="value.type_game.length !== 0">
                <h4 class="detail-row__ttl">{{translates.TYPE_GAME[config.LANG]}}</h4>
            </div>
            <div class="detail-row__content" v-if="value.type_game.length !== 0">
                <div class="game-type" v-for="(item, index) in value.type_game" :key="index">
                    <amp-img class="game-type__icon" width="17" height="20" :src="icons[item]"/>
                    {{ item }}
                </div>
            </div>
        </div>
        <div class="detail-row">
            <div class="detail-row__heading">
                <h4 class="detail-row__ttl">{{translates.PROFIT_INFORMATION[config.LANG]}}</h4>
            </div>
            <div class="detail-row__content">
                <ul class="detail-list">
                    ...
                    <li class="detail-list__item full-width">
                        <span class="detail-list__ttl">{{translates.VOLATILITY[config.LANG]}}</span>
        ...
        <div class="detail-row">
            ...
            <div class="detail-row__content">
                <ul class="detail-list">
                    <li class="detail-list__item full-width">
                        <span class="detail-list__ttl">{{translates.MAX_RATE[config.LANG]}}</span>
                        <span class="detail-list__value">{{ value.max_bet }}</span>
                    </li>
                    <li class="detail-list__item full-width">
                        <span class="detail-list__ttl">{{translates.MIN_RATE[config.LANG]}}</span>
...
<script>
    ...
    export default {
        ...
        methods: {
            detailsFilters(item) {
                return item == 1 ? this.translates.YES[this.config.LANG] : this.translates.NO[this.config.LANG]
            }
        }
    }
</script>