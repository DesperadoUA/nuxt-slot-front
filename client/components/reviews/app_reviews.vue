<template>
	...
	>
		...
		<div class="container">
			...
			>
				...
				<div class="review_item_rating">
					<div class="casino_item_rating_box">
						...
						<div
							class="casino_item_rating_full"
							v-bind:style="{ width: item.review_rating + '%' }"
						></div>
					</div>
					<div class="revie_item_box_value">
						<span class="casino_item_ratin_value">{{
			...
			<div
				class="reviews_show_more"
				v-if="value.length > numberReviewOnQuery * reviewCurrentPage"
			>
				<span class="review_btn_show" @click="reviewShowMore"
					>Показать еще</span
				>
			</div>
		</div>
	...
	computed: {
		currentReviews() {
			return this.value.slice(
				0,
				this.numberReviewOnQuery * this.reviewCurrentPage
			)
		}
	},
	methods: {
