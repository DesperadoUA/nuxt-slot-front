{
    ...
    if(btn) {
         ...
         const dataSetClosed = btn.attributes.filter((item) => item.name === 'data-translate-closed')
         const closed = dataSetClosed.length ? dataSetClosed[0].value : ""

         const dataSetWelcomeBonus = btn.attributes.filter((item) => item.name === 'data-translate-welcome-bonus')
         const welcomeBonus = dataSetWelcomeBonus.length ? dataSetWelcomeBonus[0].value : ""

         const dataSetFreeSpins = btn.attributes.filter((item) => item.name === 'data-translate-free-spins')
         const freeSpins = dataSetFreeSpins.length ? dataSetFreeSpins[0].value : ""

         const dataSetCasinoReview = btn.attributes.filter((item) => item.name === 'data-translate-casino-review')
         const casinoReview = dataSetCasinoReview.length ? dataSetCasinoReview[0].value : ""
                 ...
                 for(let i = counter * postsOnQuery; i<length; i++) {
                                ...
                                <div class="casino-item__top">
                                    <div class="casino-item__logo">
                                        <a href="${ampPrefix}${posts[i].permalink}">
                                            <amp-img src="${posts[i].thumbnail}" alt="" width="245" height="140" />
                                        </a>
                                    </div>
                                    <div class="casino-item__rating">
                                        <div class="circle-rating">
                                            <svg viewBox="0 0 36 36" width="76" height="76" class="circle-rating__chart amp_rating_circle ${classRating(posts[i])}">
                    ...
                    html += `<div class="casino-bonuses">
                                <div class="casino-bonus">
                                    <span class="casino-bonus__value colorffe600">${posts[i].bonus}</span>
                                    <span class="casino-bonus__ttl">${welcomeBonus}</span>
                                    <div class="casino-bonus__wager">${posts[i].bonus_wagering}</div>
                                </div>

                                <div class="casino-bonus">
                                    <span class="casino-bonus__value color12d4ff">${posts[i].freespins}</span>
                                    <span class="casino-bonus__ttl">${freeSpins}</span>

                                    <div class="casino-bonus__wager">${posts[i].freespins_wagering}</div>
                                </div>
                        </div>
                        ...
                        <div class="casino-item__btns">
                            ...
                            <a class="casino-item__btn --blue" href="${getRef(posts[i])}">
                                ${goTo}
                            </a>
                        </div>
                    </div>
