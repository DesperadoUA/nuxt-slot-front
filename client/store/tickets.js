    ...
    changeStatusAccordeon(state, payload) {
        ...
        state.tickets.userTickets.forEach(item => {
            if(item.id === payload.id) {
                ...
                currentItem.status_accordeon = currentItem.status_accordeon === 'close' ? 'open' : 'close'
...
export const actions = {
    ...
    async sendTicket({commit}, payload){
        ...
        if(result.data.confirm === 'ok') {
            ...
            commit('setError', 'Превышен лимит отправки жалоб, в сутки можно отправить не более 3 жалоб')
        }
    },
